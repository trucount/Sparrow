...
export function WorkspaceArea() {
  ...
  const isFileBlankOrIncomplete = (file: ProjectFile): boolean => {
    ...
    // If less than 3 non-comment lines, consider it incomplete
    if (nonCommentLines.length < 3) return true
    ...
    if (file.language === 'javascript') {
      // Very basic JS should at least have some meaningful content
      return content.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '').trim().length < 30
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  useEffect(() => {
    ...
    const handleCheckForMissingFiles = (event: CustomEvent) => {
      const { expectedFiles, receivedFiles, projectContext } = event.detail
      console.log("[v0] Checking for missing files...")
      console.log("[v0] Expected files:", expectedFiles)
      console.log("[v0] Received files:", receivedFiles)
      
      if (!currentProject) return
      
      // Find files that were expected but didn't receive proper content
      const missingFiles: string[] = []
      
      expectedFiles.forEach((filename: string) => {
        const projectFile = currentProject.files.find(f => f.name === filename)
        if (projectFile) {
          if (isFileBlankOrIncomplete(projectFile)) {
            console.log(`[v0] File ${filename} is blank or incomplete:`, projectFile.content.substring(0, 100))
            missingFiles.push(filename)
          }
        } else {
          console.log(`[v0] File ${filename} not found in project`)
          missingFiles.push(filename)
        }
      })
      ...
      missingFiles.forEach((filename, index) => {
        setTimeout(() => {
          ...
          const enhancedContext = {
            ...
            existingFiles: currentProject.files.map(f => f.name),
            filesContent: currentProject.files
              .filter(f => !isFileBlankOrIncomplete(f))
          ...
          const event = new CustomEvent("requestMissingFile", {
            detail: {
              filename: filename,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener("checkForMissingFiles", handleCheckForMissingFiles as EventListener)
    ...
    return () => {
      ...
      window.removeEventListener("checkForMissingFiles", handleCheckForMissingFiles as EventListener)
    ...
    setCurrentProject((prev) => {
      ...
      const updatedFiles = prev.files.map((file) => {
        if (file.name === filename) {
          console.log(`[v0] Updating file ${filename} with ${content.length} characters`)
          return {
            ...file,
            content,
            language,
